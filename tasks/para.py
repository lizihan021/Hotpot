...
#            s0, s1, y = loader.load_msrpara(fname)   #set it free is we decide not to use quora dataset
        ...
        gr = graph_nparray_anssel(graph_input_anssel(si0, si1, sj0, sj1, None, None, y, f0, f1, s0, s1))
    ...
    def eval(self, model):
        ...
        for gr, fname in [(self.gr, self.trainf), (self.grv, self.valf), (self.grt, self.testf)]:
            ...
            res.append(ev.eval_para(np.array(ypred[i][0] for i in range(0, ypred.shape[0])), gr['score'], fname))
        return tuple(res)
